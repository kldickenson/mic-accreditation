...
class FP_Tabs { 
	...
	public function fp_tabs( $atts, $content ) { 
		...
		if ( $atts['vertical'] == 'true' ) { 
			$html .= '<ul class="'.esc_html( $fp_globals[ 'vertical_class' ]  ).' tabs">';
		 } else { 
			$html .= '<ul class="tabs">';
		 }
	...
	public function fp_tab( $atts, $content = null ){ 
		...
		if ( $fp_globals[ 'tab_title' ]  ){ 
			...
			if ( $atts['open'] == 'true' ) { 			
				...
				$html = "<li class='".esc_html( $fp_globals[ 'open_tab' ]  ).esc_html( $class )."'><a href=#".esc_html( $fp_globals[ 'tab_id' ]  ).">". esc_html( $atts['title'] ) ."</a></li>";
			 } else { 
				if ( !empty( $atts['class'] ) ) { $class = ' class='.$atts['class']; } else { $class = ''; } 
				$html = "<li".esc_html( $class )."><a href=#".esc_html( $fp_globals[ 'tab_id' ]  ).">". esc_html( $atts['title'] ) ."</a></li>";
			 }	

		 } else { 

			if ( $atts['open'] == 'true' ) { 				
				$html = "<li id='".esc_html( $fp_globals[ 'tab_id' ]  )."Tab' class='".esc_html( $fp_globals[ 'open_tab' ]  )."'><div class='fp-tab-content'>". do_shortcode( $atts['content'] )."</div></li>";
			 } else { 
				$html = "<li id='".esc_html( $fp_globals[ 'tab_id' ]  )."Tab'><div class='fp-tab-content'>". do_shortcode( $atts['content'] )."</div></li>";
			 }	
		 }
...
class FP_Accordion { 
	...
	public function fp_accordion( $atts, $content = null ){ 
		...
		$atts = shortcode_atts( array( 
			...
			$html .= "<div class='title'><h5>".esc_html( $atts['title'] )."</h5></div>";
...
class FP_Menu { 
	...
	public function fp_menu_item( $atts, $content ){ 
		...
		$html = '<li class="has-flyout'.esc_html($class).'"><a href="'.esc_url( $atts['link'] ).'">'.esc_html( $atts['title'] ).'</a>'. do_shortcode( $atts['content'] ).'</li>';
    ...
    public function fp_submenu_item( $atts, $content ){ 
		...
		$html = '<li'.esc_html( $class ).'><a href="'.esc_html( $atts['link'] ).'">'.esc_html( $atts['title'] ).'</a>'. do_shortcode( $atts['content'] ).'</li>';
...
class FP_Button { 
	...
	public function fp_button( $atts, $content ) { 
		...
		if ( in_array( $atts['size'], array( 'tiny', 'small', 'medium', 'large' ), true ) ) { $button_classes[] = $atts['size'].' '; }
		if ( in_array( $atts['color'], array( 'secondary', 'success', 'alert' ), true ) ) { $button_classes[] = $atts['color'].' '; } 
...
class FP_Callout { 
	...
	public function fp_callout( $atts, $content ) { 
		...
		if ( in_array( $atts['color'], array( 'secondary', 'success', 'alert', 'primary' ), true ) ) { $color = $atts['color'].' '; } else { $color = ''; }
		if ( !empty( $atts['class'] ) ) { $class = $atts['class'].' '; } else { $class = ''; }
...
class FP_Dropdown { 
	...
	public function fp_dropdown( $atts, $content ) { 
		...
		$html .= '<a href="#">'. esc_html( $atts['title'] ) .'</a>';		
...
class FP_Interchange { 
	...
	public function fp_interchange( $atts, $content ) { 
		...
		$html .= '<img alt="'. esc_html( $atts['alt'] ) .'" class="hide-for-small show-for-medium-down" src="'.esc_url( $atts['medium'] ).'">';	
		$html .= '<img alt="'. esc_html( $atts['alt'] ) .'" class="show-for-small" src="'.esc_url( $atts['small'] ).'">';	
		$html .= '<img alt="'. esc_html( $atts['alt'] ) .'" class="show-for-xlarge" src="'.esc_url( $atts['large'] ).'">';		
		$html .= do_shortcode( $atts['content'] );
...
class FP_Progress { 
	...
	public function fp_progress( $atts, $content ) { 
		...
		if ( in_array( $atts['size'], array( 'small', 'large' ), true ) ) { if ( $atts['size'] == 'small' ) { $progress_classes[] = 'eight '; } else { $progress_classes[] = 'twelve '; } }
		if ( !empty( $atts['class'] ) ) { $class = $atts['class'].' '; } else { $class = ''; }
...
class FP_Reveal { 
	...
	public function fp_reveal( $atts, $content ) { 
		...
		if ( in_array( $atts['size'], array( 'expand', 'small', 'medium', 'large', 'xlarge' ), true ) ) { $size = $atts['size'].' '; } else { $size = ''; }
		if ( !empty( $atts['class'] ) ) { $class = $atts['class']. ' '; } else { $class = ''; }

		$html = '<div class="'.esc_html( $size ).esc_html( $class ).'reveal-modal" id="'.esc_html( $atts['id'] ).'">';
		$html .=  do_shortcode( $atts['content'] );
		$html .= '<a class="close-reveal-modal" aria-label="'.__( 'Close', 'fp-foundation-assistant' ).'">&#215;</a></div>';
		$html .= '<a class="fp-reveal-link" data-reveal-id="'.esc_html( $atts['id'] ).'">'.esc_html( $atts['title'] ).'</a>';
...
class FP_Tooltip { 
	...
	public function fp_tooltip( $atts, $content ) { 
		...
		if ( in_array( $atts['shape'], array( 'square' ), true ) ) { $tooltip_classes[] = ' noradius'; } 
		if ( !empty( $atts['class'] ) ) { $class = ' '.$atts['class']; } else { $class = ''; }
		if ( is_array( $tooltip_classes ) ) { $tooltip_classes = implode( " ", $tooltip_classes ); }

		$html = '<span data-tooltip aria-haspopup="true" class="has-tip'.esc_html( $tooltip_classes ).esc_html( $class ).'"  title="'.esc_html( $atts['title'] ).'">';
		$html .=  do_shortcode( $atts['content'] );
		$html .= '</span>';
...
class FP_Grid { 
	...
	public function fp_rows( $atts, $content ) { 
		...
		foreach ( $grid_columns as $grid_column ) { 
			...
			if ( !empty( $atts['up'] ) || !empty( $atts['small_up'] ) ) { 

				if ( $atts['up'] == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }
				if ( $atts['small_up'] == $grid_column[ 'number' ]  ) { $block_class[] = ' mobile-'.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }

			 } else { 

				// Not Supported in F3; Included to support compatibility between the versions;

				if ( !empty( $atts['large_up'] ) ) { 

					if ( $atts['large_up'] == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }

				 } else if ( empty( $atts['large_up'] ) && !empty( $atts['medium_up'] ) ) { 

					if ( $atts['medium_up'] == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }
				}
			}
		}		
		...
		if ( empty( $atts['small_up'] ) && empty( $atts['medium_up'] ) && empty( $atts['large_up'] ) && empty( $atts['up'] ) ) { 

			$fp_globals[ 'block_grids' ]  = 'false';

			if ($background) {
				$html = '<div class="row'.esc_html( $class ).esc_html( $bool_class ).'" style="background-image:url('.esc_url($background).');background-size:cover;">';
			} else {
			...
			if ($background) {
				$html = '<ul class="block-grid '.esc_html( $class ).esc_html( $block_class ).'" style="background-image:url('.esc_url($background).');background-size:cover;">';
			} else {
	...
	public function fp_columns( $atts, $content ){ 
		...
		$bool_options = array( 
			array( 'bool' => $atts['centered'], 'class' => 'centered ' ), 
			array( 'bool' => $atts['end'], 'class' => 'end ' ), 			
		 );

		// Not Supported in F3; Included to support compatibility between the versions;
			if ( ( empty( $atts['centered'] ) && empty( $atts['small_centered'] ) && empty( $atts['medium_centered'] ) )  && ( $atts['large_centered'] == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $atts['large_centered'], 'class' => 'centered ' );
			 } else if ( ( empty( $atts['centered'] ) && empty( $atts['small_centered'] ) )  && ( $atts['medium_centered'] == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $atts['medium_centered'], 'class' => 'centered ' );
			 } else if ( empty( $atts['centered'] ) && ( $atts['small_centered'] == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $atts['small_centered'], 'class' => 'centered ' );
			 }
		...
		foreach ( $grid_columns as $grid_column ) { 
			if ( $atts['column'] == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $atts['offset'] == $grid_column[ 'number' ]  ) { $column_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $atts['push'] == $grid_column[ 'number' ]  ) { $column_class[] = 'push-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $atts['pull'] == $grid_column[ 'number' ]  ) { $column_class[] = 'pull-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			// Not Supported in F3; Included to support compatibility between the versions;
			if ( empty( $atts['small'] ) && empty( $atts['medium'] ) && empty( $atts['large'] ) && empty( $atts['column'] ) ) { 

				if ( empty( $atts['column'] ) ) { $atts['column'] = esc_html( absint( 12 ) ); }

			} else if ( empty( $atts['small'] ) && empty( $atts['medium'] ) && empty( $atts['column'] ) ) { 

				if ( $atts['large'] == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			 } elseif ( empty( $atts['small'] ) && empty( $atts['column'] ) ) { 

				if ( $atts['medium'] == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			 } elseif ( empty( $atts['column'] ) ) { 

				if ( $atts['small'] == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			 }

			if ( empty( $atts['small_offset'] ) && empty( $atts['medium_offset'] ) && empty( $atts['offset'] ) ) { 

				if ( $atts['large_offset'] == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }

			 } elseif ( empty( $atts['small_offset'] ) && empty( $atts['offset'] ) ) { 

				if ( $atts['medium_offset'] == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }

			 } elseif ( empty( $atts['offset'] ) ) { 

				if ( $atts['small_offset'] == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }
			 }
		 }
