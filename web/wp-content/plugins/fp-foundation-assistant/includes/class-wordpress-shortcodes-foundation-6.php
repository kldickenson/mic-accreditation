...
class FP_Dropdown { 
	...
	public function fp_dropdown( $atts, $content ) { 
		...
		$html .= '<div class="'.esc_html( $class ).'dropdown-pane'.esc_html( $position ).'" id="'.esc_html( $atts['id'] ).'" data-dropdown '. esc_html($hover).'>';		
		$html .=  do_shortcode( $atts['content'] );
		$html .= '</div>';
...
class FP_Progress { 
	...
	public function fp_progress( $atts, $content ) { 
		...
		$html .=  "<p class='progress-meter-text'>".esc_html( $atts['text'] )."</p>";
		$html .=  do_shortcode( $atts['content'] );
		$html .= '</div></div>';
...
class FP_Grid { 
	...
	public function fp_rows( $atts, $content ) { 
		...
		if ($background) {
			$html = '<div class="'.esc_html( $grid_class ).esc_html( $bool_class ).esc_html( $class ).'row" '.esc_html( $equalizer ).esc_html( $row_equalizer ).' style="background-image:url('.esc_url($background).');background-size:cover;">';
		} else {
	...
	public function fp_columns( $atts, $content ){ 
		...
		$options = array( 
			...
			array( 'size' => $atts['push'], 'class' => 'small-push-' ), 
			array( 'size' => $atts['pull'], 'class' => 'small-pull-' ), 
		 );
...
class FP_Carousel { 
	...
	public function fp_carousel( $atts, $content ) { 
		...
		global $fp_globals;
		$content = str_replace( array( '<br />', '<br>' ), array( '', '' ), $content );
		$fp_globals['owl_counter']++;
		$fp_globals[ 'owl_id' ] = 'owl_id_'.uniqid().'_'.$fp_globals['owl_counter'];
